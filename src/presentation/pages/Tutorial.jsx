...
import { Play, Info, Coffee, CheckCircle, Sun, Moon } from 'lucide-react';
...
const Tutorial = ({ onComplete, theme, toggleTheme, shopName, userAvatar = 'Leo', backendStatus }) => {
  ...
  React.useEffect(() => {
    const updateML = async () => {
      ...
      setMLSuggestion(MarketService.normalizePrice(suggestion?.mlPrice ?? 5));
    };
    updateML();
  }, [conditions, mlReady]);
  ...
  const getTutorialDemand = (price) => {
    const demand = MarketService.calculateDemand(
      price,
      conditions.weather,
    ...
    const dayFiveLikeDemand = MarketService.calculateDemand(
      price,
  ...
  const handleStartDay = () => {
    const normalizedPlayerPrice = MarketService.normalizePrice(playerPrice);
    ...
    const playerSales = MarketService.calculateSales(playerDemand, inventory);
    const playerRevenue = playerSales * normalizedPlayerPrice;
    const playerProfitBreakdown = MarketService.calculateDailyProfit(playerSales, normalizedPlayerPrice, conditions.day);
    ...
    if (conditions.competitorPresent && conditions.competitorPrice) {
      ...
      if (day === 5) {
        ...
        compProfit = MarketService.calculateDailyProfit(compSales, conditions.competitorPrice, conditions.day).netProfit;
      }
    }

    // Calculate ML Results (Simulated in Tutorial)
    const mlPrice = MarketService.normalizePrice(mlSuggestion);
    const mlDemandBase = day === 7
      ? MarketService.calculateDemand(
        mlPrice,
        TUTORIAL_DAYS[5].weather,
        TUTORIAL_DAYS[5].nearbyEvent,
        TUTORIAL_DAYS[5].day,
  ...
  const handleContinue = () => {
    if (day >= 7) {
      ...
      const playerPenalty = MarketService.calculateWeekWastagePenalty(pendingInventory);
      const finalHistory = [...history];
      const startOfWeek = finalHistory[0] || { playerProfit: 0, playerRevenue: 0, playerGrossProfit: 0 };
      ...
      const weeklyStats = {
        ...
        rlTotal: 0,
        competitorTotal: 0,
        playerRevenue: (endOfWeek.playerRevenue || 0) - (startOfWeek.playerRevenue || 0),
        mlRevenue: 0,
        playerCoffeeEarnings: (endOfWeek.playerGrossProfit || 0) - (startOfWeek.playerGrossProfit || 0),
        mlCoffeeEarnings: 0,
        playerPenalty,
      ...
      <div className="w-full max-w-[95rem] mx-auto flex gap-4 lg:gap-6 flex-grow min-h-0 relative z-10 pr-2">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex flex-col gap-4 flex-grow min-h-0">
            ...
            <div className={`w-full ${useCompactTutorialLayout ? 'flex-none' : 'flex-grow min-h-0'} grid gap-4 lg:gap-6 ${useCompactTutorialLayout ? 'grid-cols-1' : 'lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]'}`}>
              <div className={`${useCompactTutorialLayout ? 'w-full h-[420px]' : 'h-full min-h-0'} bg-coffee-800/50 rounded-2xl border border-coffee-700/50 flex flex-col overflow-hidden`}>
              ...
              <div className={`${useCompactTutorialLayout ? '' : 'h-full min-h-0'} flex flex-col gap-4`}>
                {/* Animation / Simulation Box */}
                <div className={`${useCompactTutorialLayout ? 'w-full h-[320px] md:h-[360px]' : 'flex-1 min-h-[320px]'} bg-coffee-800/50 rounded-2xl flex flex-col overflow-hidden relative group transition-all duration-300`}>
                ...
                <div className={`${theme === 'theme-latte'
                  ...
                  } p-4 rounded-xl border ring-1 shadow-xl relative overflow-hidden flex flex-col gap-3 transition-all duration-300`}>
                  {/* Decor */}
                  ...
                  <motion.button
                    ...
                    className={`w-full px-4 py-3 rounded-xl text-white font-black flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest transition-all shadow-lg mt-auto ${(day <= 7 && playerPrice >= 1 && playerPrice <= 10)
