...
const AVATAR_OPTIONS = ['Leo', 'Aiden', 'Riley', 'Liam', 'Jocelyn', 'Alexander', 'Vivian', 'Avery'];
...
const LandingPage = ({ onComplete, theme, toggleTheme }) => {
  ...
  return (
    <div className={`h-full bg-coffee-900 text-coffee-100 flex flex-col items-center justify-start p-4 md:p-8 relative overflow-hidden transition-colors duration-500 ${theme}`}>
          ...
          <AnimatePresence mode="wait">
            {!currentStory.isEnd ? (
              <motion.img
                ...
                className={step === 3
                  ? "w-full h-auto max-w-full max-h-full object-contain drop-shadow-2xl rounded-3xl"
                    ...
                    <div className="grid grid-cols-4 gap-3">
                      {AVATAR_OPTIONS.map((seed) => (
                        <motion.button
                          ...
                          className={`mx-auto w-14 h-14 rounded-full border-2 p-0.5 transition-all shadow-md ${selectedAvatar === seed ? 'border-amber-500 bg-amber-500/20 shadow-amber-500/20' : 'border-coffee-700 bg-coffee-800/50 hover:border-coffee-500'}`}
                        >
