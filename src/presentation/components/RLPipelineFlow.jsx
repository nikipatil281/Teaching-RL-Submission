...
const NET_REWARDS_TO_AGENT_ARROW_OUTSET = 18;
const STATE_TO_AGENT_ARROW_OUTSET = 18;
const AGENT_TO_ACTION_ARROW_OUTSET = 0;
const ACTION_TO_FEEDBACK_ARROW_OUTSET = 0;
const ENVIRONMENT_TO_POLICY_ARROW_OUTSET = 0;
const ENVIRONMENT_TO_POLICY_SOURCE_LEFT_PERCENT = 72;
const ENVIRONMENT_TO_POLICY_START_X_OFFSET = -50;
const ENVIRONMENT_TO_POLICY_START_Y_OFFSET = 115;
const POLICY_TO_ENVIRONMENT_ARROW_OUTSET = 0;
const BOTTOM_TARGET_OUTSET = 8;
const POLICY_TO_ENVIRONMENT_START_X_OFFSET = 45;
const POLICY_TO_ENVIRONMENT_START_Y_OFFSET = 0;
const POLICY_TO_ENVIRONMENT_ELBOW_X_OFFSET = 96;
const POLICY_TO_ENVIRONMENT_END_Y_OFFSET = 115;
const ACTION_TO_FEEDBACK_START_X_OFFSET = 48;
const ACTION_TO_FEEDBACK_START_Y_OFFSET = 0;
const ACTION_TO_FEEDBACK_ELBOW_X_OFFSET = 0;
...
const buildEdge = ({ id, source, target, sourceHandle, targetHandle, color, dashed = false, animated = false, type = 'visibleSmoothStep', zIndex, targetOutset, sourceXOffset, sourceYOffset }) => ({
    id,
    source,
    target,
    sourceHandle,
...
const getOutsetTarget = (targetX, targetY, targetPosition, outset = ARROW_TARGET_OUTSET) => {
    if (targetPosition === Position.Left) return { targetX: targetX - outset, targetY };
    if (targetPosition === Position.Right) return { targetX: targetX + outset, targetY };
    if (targetPosition === Position.Top) return { targetX, targetY: targetY - outset };
    if (targetPosition === Position.Bottom) return { targetX, targetY: targetY + outset };
...
const DESKTOP_EDGES = [
    buildEdge({
        ...
        color: '#34d399',
        animated: true,
        targetOutset: FEEDBACK_TO_NET_REWARDS_ARROW_OUTSET,
    }),
    buildEdge({
        ...
        color: '#5eead4',
        animated: true,
        targetOutset: FEEDBACK_TO_STATE_ARROW_OUTSET,
    }),
    buildEdge({
        ...
        color: '#34d399',
        animated: true,
        targetOutset: NET_REWARDS_TO_AGENT_ARROW_OUTSET,
    }),
    buildEdge({
        id: 'state-to-agent',
        source: 'state',
        ...
        color: '#7dd3fc',
        animated: true,
        targetOutset: STATE_TO_AGENT_ARROW_OUTSET,
    }),
    buildEdge({
        id: 'agent-to-action',
        source: 'agent',
        ...
        color: '#f9a8d4',
        animated: true,
        targetOutset: AGENT_TO_ACTION_ARROW_OUTSET,
    }),
    buildEdge({
        ...
        color: '#fdba74',
        animated: true,
        type: 'actionToFeedback',
        zIndex: 80,
        targetOutset: ACTION_TO_FEEDBACK_ARROW_OUTSET,
    }),
    buildEdge({
        id: 'environment-to-policy',
        source: 'environment',
        target: 'policy',
        sourceHandle: 'source-bottom-policy',
        targetHandle: 'target-left',
        color: '#d6bcaa',
        animated: true,
        targetOutset: ENVIRONMENT_TO_POLICY_ARROW_OUTSET,
        sourceXOffset: ENVIRONMENT_TO_POLICY_START_X_OFFSET,
        sourceYOffset: ENVIRONMENT_TO_POLICY_START_Y_OFFSET,
    }),
    buildEdge({
        ...
        type: 'singleBend',
        zIndex: 80,
        targetOutset: POLICY_TO_ENVIRONMENT_ARROW_OUTSET,
    }),
];

const SingleBendEdge = ({ sourceX, sourceY, targetY, markerEnd, style, data }) => {
    const startX = sourceX + POLICY_TO_ENVIRONMENT_START_X_OFFSET;
    const startY = sourceY + POLICY_TO_ENVIRONMENT_START_Y_OFFSET;
    const elbowX = startX + POLICY_TO_ENVIRONMENT_ELBOW_X_OFFSET;
    const endY = targetY + POLICY_TO_ENVIRONMENT_END_Y_OFFSET + (data?.targetOutset ?? 0);
    const edgePath = `M ${startX} ${startY} H ${elbowX} V ${endY}`;

    return <BaseEdge path={edgePath} markerEnd={markerEnd} style={style} />;
};

const VisibleSmoothStepEdge = ({ sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, style, data }) => {
    const adjustedSourceX = sourceX + (data?.sourceXOffset ?? 0);
    const adjustedSourceY = sourceY + (data?.sourceYOffset ?? 0);
    const outsetTarget = getOutsetTarget(targetX, targetY, targetPosition, data?.targetOutset ?? ARROW_TARGET_OUTSET);
    const [edgePath] = getSmoothStepPath({
        sourceX: adjustedSourceX,
        sourceY: adjustedSourceY,
        sourcePosition,
        targetX: outsetTarget.targetX,
        targetY: outsetTarget.targetY,
        targetPosition,
    });
...
const BottomTargetSmoothStepEdge = ({ sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, markerEnd, style }) => {
    const [edgePath] = getSmoothStepPath({
        sourceX,
        sourceY,
        sourcePosition,
        targetX,
        targetY: targetY + BOTTOM_TARGET_OUTSET,
        targetPosition,
    });
...
const ActionToFeedbackEdge = ({ sourceX, sourceY, targetX, targetY, markerEnd, style, data }) => {
    const startX = sourceX + ACTION_TO_FEEDBACK_START_X_OFFSET;
    const startY = sourceY + ACTION_TO_FEEDBACK_START_Y_OFFSET;
    const endX = targetX + ACTION_TO_FEEDBACK_END_X_OFFSET;
    const endY = targetY + ACTION_TO_FEEDBACK_END_Y_OFFSET + (data?.targetOutset ?? 0);
    const elbowX = endX + ACTION_TO_FEEDBACK_ELBOW_X_OFFSET;
    const preEndY = startY + ((endY - startY) * 0.7);
    const edgePath = `M ${startX} ${startY} H ${elbowX} V ${preEndY} H ${endX} V ${endY}`;
...
const renderHandles = () => (
    <>
        ...
        <Handle id="source-bottom-policy" type="source" position={Position.Bottom} className={HANDLE_CLASS} style={{ left: `${ENVIRONMENT_TO_POLICY_SOURCE_LEFT_PERCENT}%` }} />
...
const GroupNode = ({ data, positionAbsoluteX = 0, positionAbsoluteY = 0 }) => {
    const { isLight } = useContext(PipelineThemeContext);
    const { activeKeys, pinnedKeys, openDetail, closeDetail, togglePinnedDetail } = useContext(SharedDetailContext);
    const styles = isLight
    ...
    const isSharedDetailOpen = hasSharedDetail && (activeKeys.includes(data.sharedDetailKey) || pinnedKeys.includes(data.sharedDetailKey));
    ...
    return (
        ...
        >
            ...
            <div
                ...
                onPointerEnter={hasSharedDetail ? () => openDetail(data.sharedDetailKey) : undefined}
                onPointerLeave={hasSharedDetail ? () => closeDetail(data.sharedDetailKey) : undefined}
                onFocus={hasSharedDetail ? () => openDetail(data.sharedDetailKey) : undefined}
                onBlur={hasSharedDetail ? () => closeDetail(data.sharedDetailKey) : undefined}
                style={hasSharedDetail ? { pointerEvents: 'all', zIndex: data.sharedDetailAnchor && isSharedDetailOpen ? 220 : undefined } : undefined}
            >
                <button
                    type="button"
                    onClick={hasSharedDetail ? () => togglePinnedDetail(data.sharedDetailKey) : undefined}
...
const InteractiveCardNode = ({ data }) => {
    ...
    const isOpen = activeKeys.includes(detailKey) || pinnedKeys.includes(detailKey);
    ...
    return (
        <div
            ...
            onPointerEnter={() => openDetail(detailKey)}
            onPointerLeave={() => closeDetail(detailKey)}
            onFocus={() => openDetail(detailKey)}
            onBlur={() => closeDetail(detailKey)}
            style={{ pointerEvents: 'all' }}
        >
            {renderHandles()}
            <button
                type="button"
                onClick={() => togglePinnedDetail(detailKey)}
...
const edgeTypes = {
    actionToFeedback: ActionToFeedbackEdge,
    bottomTargetSmoothStep: BottomTargetSmoothStepEdge,
    singleBend: SingleBendEdge,
    visibleSmoothStep: VisibleSmoothStepEdge,
};
...
const RLPipelineFlow = ({ theme }) => {
    ...
    useEffect(() => {
        ...
        const observer = new ResizeObserver(([entry]) => {
            ...
            setLensPosition((current) => (
                current.x === LENS_WIDTH / 2 && current.y === LENS_HEIGHT / 2
                    ? { x: nextWidth / 2, y: nextHeight / 2 }
                    : current
            ));
        });
    ...
    const lensViewport = viewport
        ? {
            ...
            y: (viewport.y * LENS_ZOOM) + lensTranslateY,
            zoom: viewport.zoom * LENS_ZOOM,
        }
        : null;
    ...
    const closeDetail = (key) => {
        setActiveKeys((current) => current.filter((activeKey) => activeKey !== key));
    };
    const togglePinnedDetail = (key) => {
        setPinnedKeys((current) => (
            current.includes(key)
                ? current.filter((pinnedKey) => pinnedKey !== key)
                : [...current, key]
        ));
    };
    ...
    return (
        <PipelineThemeContext.Provider value={{ isLight }}>
            <SharedDetailContext.Provider value={{ activeKeys, pinnedKeys, openDetail, closeDetail, togglePinnedDetail }}>
                <div className="w-full">
                    ...
                    >
                        ...
                        {isLensEnabled && isLensVisible && lensViewport && surfaceSize.width > 0 && surfaceSize.height > 0 ? (
                            <div
                                className="pointer-events-none absolute z-40 overflow-hidden rounded-[22px] border border-amber-200/70 shadow-[0_22px_55px_rgba(15,23,42,0.38)]"
                                style={{
                                    width: LENS_WIDTH,
                                    height: LENS_HEIGHT,
                            ...
                            >
                                ...
                                <div className="pointer-events-none absolute inset-0 rounded-[22px] ring-1 ring-white/65 ring-offset-2 ring-offset-transparent" />
                                <div className="pointer-events-none absolute inset-3 rounded-[14px] border border-amber-100/60 bg-white/5 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.12)]" />
                            </div>
                        ) : null}
                    </div>
                </div>
